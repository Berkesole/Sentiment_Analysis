...
class DictClassifier:
    ...
    def __divide_sentence_into_clauses(self, the_sentence):
        the_clauses = self.__split_sentence(the_sentence)  # 划分成子句，每个子句是一段文本加上一个/多个连续的标点
        ...
        # 就[\u4e00-\u9fa5] 后面应该是 ? 表示匹配0次或1次
        pattern = re.compile(
            r"([，%。、！；？?,!～~.… ]*)([\u4e00-\u9fa5]*?(如果|要是|希望).+就[\u4e00-\u9fa5]?(好|完美)了[，%。、！；？?,!～~.… ]+)")
        match = re.search(pattern, the_sentence.strip())
        # print(match.group(2))
        # group()用来提出分组截获的字符串
        # 这里的意思是删除重复的匹配项？？？
        ...
        the_clauses[-1] = the_clauses[-1][:-1]  # 去掉最后一个子句后额外加的百分号
        return the_clauses
