...
def load_file_and_preprocessing():
    neg_sents = pd.read_excel('../data/neg.xls', header=None)[0]
    pos_sents = pd.read_excel('../data/pos.xls', header=None)[0]

    pos_words = [jieba.cut(x) for x in pos_sents]
    neg_words = [jieba.cut(x) for x in neg_sents]
...
def get_train_vecs(x_train, x_test):
    n_dim = 300
    # 初始化模型和词表
    imdb_w2v = Word2Vec(x_train, min_count=10)
