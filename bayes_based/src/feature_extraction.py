...
class ChiSquare:
    ...
    @staticmethod
    def __calculate(n_ii, n_ix, n_xi, n_xx):  # word在pos出现的次数、word在所有出现的次数、所有word的pos总次数、所有words的总次数
        n_ii = n_ii  # word在pos出现的次数
        n_io = n_xi - n_ii  # 其他词在pos出现的次数
        n_oi = n_ix - n_ii  # word在neg出现的次数
        n_oo = n_xx - n_ii - n_oi - n_io  # 其他词在neg出现的次数
